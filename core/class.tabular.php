		...
		*/
		...
		public function authenticate_action_requests() {
			...
			if ( $this->doing_bulk_action && $this->current_action() == 'delete' ) {
				...
				if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'wpgmp-nonce' ) ) {
					wp_die( 'You are not allowed to save changes!' );
				}
			}
					...
					<div class="fc-back">
						...
						<div class="fc-12">
							...
							<div class="wpgmp_menu_title">
								...
								<?php
								echo esc_html( $this->translation['manage_heading'] );
								...
								<span><a class="fc-new-link" target="_blank" href="<?php echo esc_url( $fc_add_record_url ); ?>"><?php echo esc_html( $this->translation['add_button'] ); ?></a>
						...
						<div class="wpgmp-overview">
							...
							<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=' . $this->admin_listing_page_name ) ); ?>" <?php echo wp_kses_post( $form_attr ); ?> >
		...
		function get_user_selected_records() {
			...
			$ids = isset( $_REQUEST['id'] ) ? array_map( 'intval', (array) wp_unslash( $_REQUEST['id'] ) ) : array();
		...
		function process_bulk_action() {
			...
			} elseif ( 'export_location_csv' === $this->current_action() || 'export_csv' === $this->current_action() && ! empty( $ids ) ) {
		...
		function prepare_items() {
			...
			if ( ! $this->noSql ) {
				...
				if ( $this->admin_listing_page_name == $page && '' != $search ) {
					...
					$prepared_query_data = array(
						'query'        => 'SELECT * FROM ' . $this->table,
						'placeholders' => $prepare_query_with_placeholders,
						'values'       => $prepare_args_values,
					);
				...
				} elseif ( isset( $_GET['orderby'] ) && ! empty( $_GET['orderby'] ) && isset( $_GET['order'] ) && ! empty( $_GET['order'] ) ) {
