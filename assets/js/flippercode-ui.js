...
jQuery( document ).ready(
	function($) {
		...
		$( '.wp-list-table' ).find( ':checkbox' ).change(
			function() {
				let temp          = [];
		...
		$( document ).on(
			...
			function(e) {
				...
				e.preventDefault();
				console.log( $( ".fc_manage_screen_form" ) );
				console.log( "Here" );
		...
		jQuery( '.fc_backend_custom_action' ).on(
			'shown.bs.modal',
			function(event) {
		...
		if ($overviewPage) {
			...
			$( '#user-suggestion,#purchase_code' ).keyup(
				function() {
			...
			$( 'body' ).on(
				'click',
				'input[name="wpgmp_update_plugin"]',
				function(e) {
					e.preventDefault();
...
jQuery( document ).ready(
	function($) {
		...
		$( '.yes-remove-current-template' ).on(
			"click",
			function() {
		...
		$( '.fc-main' ).find( '[data-toggle="tab"]' ).click(
			function(e) {
				e.preventDefault();
